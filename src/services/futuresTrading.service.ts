...
class FuturesTradingService {
  ...
  async initializeExchange(
    ...
    try {
      console.log(`🔌 Initializing ${credentials.exchange} connection with CCXT...`);
      ...
      if (connectionTest.success) {
        ...
        console.log(`✅ Successfully connected to ${credentials.exchange} via CCXT`);
        ...
        console.error(
          `❌ CCXT connection failed for ${credentials.exchange}:`,
      ...
      console.error('💥 Exception in initializeExchange:', errorMessage);
  ...
  async getSpotBalance(exchange: 'binance' | 'gate'): Promise<any> {
    ...
    try {
      console.log(`📊 Fetching spot balance from ${exchange} via CCXT...`);
      ...
      if (!result.success) {
        console.error(`❌ Failed to fetch spot balance from ${exchange}:`, result.error);
        return {
          success: false,
          error: result.error,
        };
      ...
      console.log(`✅ Successfully fetched spot balance from ${exchange}`);
      return {
        success: true,
        data: result.data,
      };
      ...
      console.error('💥 Exception in getSpotBalance:', errorMessage);
  ...
  async getAccountBalance(exchange: 'binance' | 'gate'): Promise<AccountBalance> {
    ...
    try {
      console.log(`📊 Fetching futures balance from ${exchange} via CCXT...`);
      ...
      console.log(`✅ Successfully fetched futures balance from ${exchange}`);
      ...
      console.error('💥 Exception in getAccountBalance:', errorMessage);
      throw new Error(errorMessage);
    }
  }
  ...
  async getPositions(exchange: 'binance' | 'gate', symbol?: string): Promise<Position[]> {
    ...
    if (!credentials) {
      console.warn(`⚠️ No credentials found for ${exchange}`);
      return [];
    }

    try {
      console.log(
        `📊 Fetching positions from ${exchange}${symbol ? ` for symbol ${symbol}` : ''} via CCXT...`
      );
      ...
      if (!result.success) {
        console.error(`❌ Failed to fetch positions from ${exchange} via CCXT:`, result.error);
        return [];
      }

      console.log(
        `✅ Successfully fetched ${result.data?.length || 0} positions from ${exchange} via CCXT`
      );
      ...
      if (symbol) {
        ...
        console.log(
          `🔍 Filtered ${positions.length} positions to ${filtered.length} for symbol ${symbol}`
        );
        return filtered;
      }
      ...
      console.error(`💥 Exception while fetching positions from ${exchange}:`, errorMessage);
      return [];
    }
  }
  ...
  async openPosition(order: TradingOrder): Promise<any> {
    ...
    try {
      console.log(
        `📝 Opening position: ${order.side.toUpperCase()} ${order.amount} ${order.symbol} on ${order.exchange} via CCXT...`
      );
      ...
      if (!result.success) {
        console.error(`❌ Order placement failed on ${order.exchange}:`, result.error);
        return {
          success: false,
      ...
      console.log(`✅ Position opened successfully on ${order.exchange}:`, result.data);
      ...
      console.error('💥 Exception in openPosition:', errorMessage);
  ...
  async closePosition(exchange: 'binance' | 'gate', symbol: string): Promise<any> {
    ...
    try {
      console.log(`🔴 Closing position for ${symbol} on ${exchange} via CCXT...`);
      ...
      if (!position) {
        console.warn(`⚠️ No open position found for ${symbol} on ${exchange}`);
        return {
          success: false,
      ...
      if (!result.success) {
        console.error(`❌ Failed to close position for ${symbol}:`, result.error);
        return {
          success: false,
      ...
      console.log(`✅ Position closed successfully for ${symbol}:`, result.data);
      ...
      console.error('💥 Exception in closePosition:', errorMessage);
  ...
  async getMarketData(exchange: 'binance' | 'gate', symbol: string): Promise<MarketData> {
    ...
    try {
      console.log(`📊 Fetching market data for ${symbol} from ${exchange} via CCXT...`);
      ...
      console.log(`✅ Market data fetched successfully for ${symbol} from ${exchange}`);
      return result.data!;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error(`❌ Failed to fetch market data for ${symbol} from ${exchange}:`, errorMessage);
  ...
  private async getBinanceMarketData(symbol: string): Promise<MarketData> {
    console.warn('⚠️ getBinanceMarketData is deprecated. Use CCXT-based getMarketData instead.');
  ...
  private async getGateMarketData(symbol: string): Promise<MarketData> {
    console.warn('⚠️ getGateMarketData is deprecated. Use CCXT-based getMarketData instead.');
  ...
  async getOpenOrders(exchange: 'binance' | 'gate', symbol?: string): Promise<any[]> {
    ...
    try {
      console.log(
        `📊 Fetching open orders from ${exchange}${symbol ? ` for ${symbol}` : ''} via CCXT...`
      );
      ...
      if (!result.success) {
        console.error(`❌ Failed to fetch open orders from ${exchange}:`, result.error);
        return [];
      }

      console.log(`✅ Fetched ${result.data?.length || 0} open orders from ${exchange}`);
      return result.data || [];
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('💥 Exception in getOpenOrders:', errorMessage);
      return [];
    }
  }
  ...
  async cancelOrder(exchange: 'binance' | 'gate', orderId: string, symbol: string): Promise<any> {
    ...
    try {
      console.log(`❌ Cancelling order ${orderId} for ${symbol} on ${exchange} via CCXT...`);
      ...
      console.error('💥 Exception in cancelOrder:', errorMessage);
  ...
  async testPositionFetching(exchange: 'binance' | 'gate'): Promise<{
    ...
    try {
      console.log(`🔍 Testing CCXT position fetching for ${exchange}...`);
      ...
      console.log(`🔑 Found credentials for ${exchange}`);

      // Get Exchange Service diagnostics
      const serviceInfo = ccxtExchangeService.getDiagnostics();
      console.log(`🔧 Service Version: ${serviceInfo.serviceVersion}`);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('💥 CCXT position fetching test failed:', errorMessage);
  ...
  async getMultipleMarketData(
    ...
    console.log(
      `📊 Fetching market data for ${symbols.length} symbols from ${exchange} via CCXT...`
    );
    ...
    try {
      ...
      console.log(
        `✅ Successfully fetched market data for ${validResults.length}/${symbols.length} symbols`
      );
      return validResults;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('💥 Exception in getMultipleMarketData:', errorMessage);
      return [];
    }
  }
